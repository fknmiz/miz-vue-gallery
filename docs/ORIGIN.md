...
**miz-vue-gallery** came from a familiar enterprise pattern: important systems were technically functional but operationally under-explained. People could often find the inputs, but still struggle to form a clear next move.
...
This repo also avoids trying to be a vague platform for everything. Its value comes from being opinionated about a real problem: Vue 3 photography portfolio gallery with animated category filtering, lightbox viewer, and lazy-loaded image grid. Portfolio piece demonstrating Vue 3 Composition API.
